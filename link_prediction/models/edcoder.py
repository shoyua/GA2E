...
class PreModel(nn.Module):
    ...
    def mask_attr_prediction(self, g):  # , num, y_onehot):
        ...
        return recovered_graph_list, mean, var
    ...
    def forward(self, graph_list):
        ...
        graph_list=[dgl.to_bidirected(graph,copy_ndata=True) for graph in graph_list]
        g=dgl.batch(graph_list).to(next(self.parameters()).device)
        # x=g.ndata["feat"]
        # noise = torch.randn(x.shape)
        # noise = noise.to(x.device)
        # g.ndata["feat"]+=noise * 0.01
